...
def test_trajectory_records_full_request_messages() -> None:
    ...
    _record_llm_request_trajectory(payload, {"submission_id": "submission-1"}, "model-a", 4096, 1000000)

    req = json.loads(broker_trajectory_path(job_id, "generation").read_text(encoding="utf-8").splitlines()[0])
    assert req["type"] == "llm_request"
    assert req["message_count"] == 2
    # The full prompt/context is captured (the gap full-capture closes).
    assert [m["role"] for m in req["messages"]] == ["system", "user"]
    assert "You are an agent." in req["messages"][0]["content"]
