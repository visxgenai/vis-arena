...
def _run_tool_loop(system_prompt: str, user_prompt: str, tool_root: Path, purpose: str) -> dict[str, Any]:
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                "name": "playwright",
                "description": "Run a PYTHON Playwright script (NOT JavaScript) to open the artifact and inspect it — DOM, screenshots, console logs, interactions. Use playwright.sync_api; the artifact URL is in the VIS_ARENA_ARTIFACT_URL env var. You MUST actually load and read the rendered page before scoring it.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "script": {
                            "type": "string",
                            "description": "A PYTHON script using playwright.sync_api (sync_playwright). Do NOT write JavaScript — no require()/const/await/=>.",
                        },
                        "cwd": {"type": "string", "description": "Optional working directory."},
                    },
                    "required": ["script"],
...
_PY_PLAYWRIGHT_EXAMPLE = (
    "from playwright.sync_api import sync_playwright\n"
    "import os\n"
    "with sync_playwright() as p:\n"
    "    browser = p.chromium.launch()\n"
    "    page = browser.new_page()\n"
    "    page.goto(os.environ['VIS_ARENA_ARTIFACT_URL'])\n"
...
def _looks_like_javascript(script: str) -> bool:
    if "require(" in script or "=>" in script or "document." in script:
        return True
    for line in script.splitlines():
        if line.strip().startswith(("const ", "let ", "var ", "async function", "function ")):
            return True
    return False


def _run_playwright(script: str, cwd: Path) -> str:
    # This tool executes the script as PYTHON. LLMs frequently default to JavaScript
    # Playwright (require/const/await/=>), which fails as a SyntaxError and leaves the
    # eval scoring blind. Detect that and steer back to Python so renders are reliable.
    if _looks_like_javascript(script):
        return (
            "Tool error: this looks like a JavaScript Playwright script, but this tool runs a "
            "PYTHON script. Rewrite it with playwright.sync_api — do NOT use require/const/await. "
            f"Example:\n{_PY_PLAYWRIGHT_EXAMPLE}"
        )
