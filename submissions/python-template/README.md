...
The template splits the arena contract from the agent implementation so you can
plug in your own agent without touching the protocol layer:

- **`agent.py`** — the arena contract. Handles argparse, JSON output shapes,
  exit codes. Dispatches to four hooks. You usually do NOT edit this file.
- **`example_agent.py`** — an example implementation (OpenAI tool loop). Replace
  the bodies of these functions with your own agent:

  ```python
  info()                                              -> dict
  models()                                            -> dict   # optional
  generate(task_path, data_dir, source_dir, dist_dir) -> dict   # write index.html
  evaluate(task_path, data_dir, source_dir, dist_dir) -> dict   # score + criteria
  ```

  Keep the function names and signatures. Everything else — prompts, LLM client,
  tools — is yours to delete or rewrite.
