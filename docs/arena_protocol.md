...
```json
{
  ...
  "browser": {
    "tool": "playwright",
    "entrypoint_url": "file:///run/built/index.html",
    "viewports": [
      {
        "width": 1440,
        "height": 900,
...
- `run_playwright_script(script: str)`: execute a Playwright script against `built/`.
- `browser_snapshot()`: capture DOM summary and screenshots.
- `read_file(path)`, `list_files(path)`, `search_files(pattern)`: inspect source and built artifacts.
